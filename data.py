...
def _write_timestep_row(
    ...
    row = [
        str(timestep),
        f"{float(pose_origin[0]):.3f}",
        f"{float(pose_origin[1]):.3f}",
        f"{float(pose_origin[2]):.3f}",
        f"{float(yaw_deg):.3f}",
        f"{float(pose_basis[0, 0]):.6f}",
        f"{float(pose_basis[0, 1]):.6f}",
        f"{float(pose_basis[0, 2]):.6f}",
        f"{float(pose_basis[1, 0]):.6f}",
        f"{float(pose_basis[1, 1]):.6f}",
        f"{float(pose_basis[1, 2]):.6f}",
        f"{float(pose_basis[2, 0]):.6f}",
        f"{float(pose_basis[2, 1]):.6f}",
        f"{float(pose_basis[2, 2]):.6f}",
